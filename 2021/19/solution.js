function learn(time, courses) {
  let max = -1;
  let best_indices = [null, null];
  for (let k=0; k<courses.length-1; k++) {
    for (let kk=k+1; kk<courses.length; kk++) {
      let act_time = courses[k] + courses[kk]; 
      if (act_time == time)       return [k, kk];
      else if (act_time < time && act_time > max) {
        max = act_time;
        best_indices = [k, kk];
      }
    }
